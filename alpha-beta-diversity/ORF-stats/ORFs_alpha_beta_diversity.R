...
tax_table <- read.table(here::here("GitHub", "PacBio-Marine-Metagenomes", "ORF-stats","metaflye_hybrid_illumina_dfs.txt"), sep="\t", header=TRUE)
tax_table <- tax_table %>% remove_rownames %>% column_to_rownames('X')
names(tax_table) <- sub("^X", "", names(tax_table))
names(tax_table) <- gsub("\\.", "-", names(tax_table))
tax_table <- t(tax_table)

metadata <- read.csv(here::here("GitHub", "PacBio-Marine-Metagenomes", "ORF-stats","ORFs_diversity_metadata.csv"))
...
pal <- c("#ff7f00", "#1f78b4", "#00cc00")

plot_shandiv <- ggplot(viol_plot_df, 
                       aes(x = sequencing_platform, y = shan_div, fill = sequencing_platform, label = eDNA_sample)) +
  geom_violin(width = 1.0, color = "black", trim=FALSE) +
  # geom_text_repel(size=2, max.overlaps=22) + # to label points
...
ggsave(path = "/Users/nastassia.patin/GitHub/PacBio-Marine-Metagenomes/ORF-stats/",
       filename = "PacBio_Illumina_ORFs_alphadiv.pdf", plot_shandiv,width = 4,height = 4)

write.csv(viol_plot_df, "/Users/nastassia.patin/GitHub/PacBio-Marine-Metagenomes/ORF-stats/ORF_shannondiv.csv", row.names = FALSE)

### Run PERMANOVA

# Load metadata
pca_metadata <- read.csv(here::here("GitHub", "PacBio-Marine-Metagenomes", "ORF-stats","permanova_metadata.csv"))

## Load DEICODE distance matrix

distance <- read_qza(here::here("GitHub", "PacBio-Marine-Metagenomes", "ORF-stats", "deicode", "distance_matrix.qza"))
...
pco <- read_qza(here::here("GitHub", "PacBio-Marine-Metagenomes", "ORF-stats", "deicode", "biplot.qza"))
...
sequencing_colors <- c("Illumina" = "#ff7f00", "PacBio" = "#1f78b4", "Illumina-PacBio" = "#00cc00")
...
ggsave(path = "/Users/nastassia.patin/GitHub/PacBio-Marine-Metagenomes/ORF-stats/",
       filename = "Lasker2019_illumina_metaflye_hybrid_PCA_pairs_labeled.png", PCA_pairs_labeled,width = 7,height = 5)

# Create a legend manually
PCA_legend <- ggplot(pca_data) +
  # metaFlye vs. SPAdes vs. hybrid
  annotate("text",label = "Sequencing  Platform", x = 0, y = 3.2,size = 8,adj = 0)+
  annotate("point", x = 0.2, y = 2.8, shape = 21, colour = "black", fill = "#ff7f00", size = 7, stroke = 2)+ 
  annotate("text",label = "Illumina", x = 0.6, y = 2.8,size = 7,adj = 0)+
  annotate("point", x = 0.2, y = 2.4, shape = 21, colour = "black", fill = "#1f78b4", size = 7, stroke = 2)+
  annotate("text",label = "PacBio", x = 0.6, y = 2.4,size = 7,adj = 0)+
  annotate("point", x = 0.2, y = 2.0, shape = 21, colour = "black", fill = "#00cc00", size = 7, stroke = 2)+
  annotate("text",label = "PacBio+Illumina", x = 0.6, y = 2.0,size = 7,adj = 0)+
  # Depth
  annotate("text",label = "Depth Group (m)", x = 0, y = 1.4, size = 8,adj = 0)+ # Title 
  annotate("point", x = 0.2, y = 1.0, shape = 21, colour = "black", fill = "black", size = 6, stroke = 3)+ # circle point
  annotate("text",label = "5-10 m", x = 0.7, y = 1.0, size = 6,adj = 0)+
  annotate("point", x = 0.2, y = 0.6, shape = 22, colour = "black", fill = "black", size = 6, stroke = 3)+ # triangle point
  annotate("text",label = "11-28 m", x = 0.7, y = 0.6, size = 6,adj = 0)+
  annotate("point", x = 0.2, y = 0.2, shape = 24, colour = "black", fill = "black", size = 6, stroke = 3)+ # square point
  annotate("text",label = "45-50 m", x = 0.7, y = 0.2, size = 6,adj = 0)+
  xlim(0,10)+
  ylim(0.2,3.2)+
...
combined_PCA_gg <- ggpubr::ggarrange(ggpubr::ggarrange(PCA_pairs_labeled, ncol = 1, nrow = 1,
                                                       labels = c("ORF PCA"), label.x = 0.05, label.y = 0.985,
...
ggsave(here("GitHub", "PacBio-Marine-Metagenomes", "ORF-stats", "ORFs_aitchison_histogram.pdf"), histograms_ALL,
       height = 11, width = 8)
